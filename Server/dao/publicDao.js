...
module.exports = class PublicDao extends Dao {
  ...
  getPublicEvents(callback) {
    super.query(
      'SELECT e.*, l.address, l.name as location_name, l.postcode FROM event e LEFT JOIN location l USING(location_id) LEFT JOIN event_organiser USING(event_id) LEFT JOIN organiser o USING(organiser_id) WHERE end > now() - INTERVAL 1 month AND e.is_public IS TRUE AND o.verified IS TRUE ORDER BY start',
      [],
      callback,
    );
  }
